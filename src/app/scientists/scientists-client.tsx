...
export function ScientistsClient() {
  return (
    <div>
      <PageHeader
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {scientists.map((scientist, i) => (
            <motion.a
              ...
              aria-label={`${scientist.name} — ${scientist.state} (opens Wikipedia)`}
            >
              <div className={`h-20 bg-gradient-to-r ${gradients[i % gradients.length]}`} />
              <div className="px-5 pb-5">
                <div className="-mt-8 mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-white text-2xl font-bold text-green-700 shadow-lg border-4 border-white" aria-hidden="true">
                  {scientist.name.charAt(0)}
                </div>
                <h3 className="font-semibold text-gray-900 group-hover:text-primary transition-colors">{scientist.name}</h3>
