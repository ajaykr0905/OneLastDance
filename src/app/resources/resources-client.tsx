            ...
            return (
              <section key={cat.category} aria-labelledby={`cat-${ci}`}>
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <div>
                    <h2 id={`cat-${ci}`} className="text-xl font-bold text-gray-900">{cat.category}</h2>
                  ...
                  {cat.resources.map((res, ri) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="font-semibold text-gray-900 group-hover:text-primary transition-colors">{res.title}</h3>
                        <ArrowUpRight className="h-4 w-4 shrink-0 text-gray-300 group-hover:text-primary transition-colors" aria-hidden="true" />
                      </div>
                      <p className="mt-2 flex-1 text-sm text-gray-600 leading-relaxed">{res.description}</p>
                      <div className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary">
                        Visit Resource <ExternalLink className="h-3 w-3" aria-hidden="true" />
                      </div>
                    </motion.a>
                  ))}
                </div>
